...
class CosClassifier(torch.nn.Module):
    def __init__(self, input_size, device="cpu", out_neurons=45):
        super().__init__()
...
def compute_loss_inter(classifier):
    W = classifier.weight
    WW = W.T @ W
    WW_clamp = torch.clamp(WW, min=0.0)
    I = torch.eye(WW_clamp.shape[0]).to(W.device)
    loss_inter = torch.norm(WW_clamp - I).square() / float(classifier.weight.shape[0])
...
class LID(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        with torch.no_grad():
            teacher_embeddings = self.modules.teacher_embedding_model(feats, lens)
            teacher_embeddings = self.modules.teacher_mean_var_norm_emb(
                x=teacher_embeddings, 
                lengths=torch.ones(teacher_embeddings.shape[0], device=teacher_embeddings.device)
            )
            teacher_outputs = self.modules.teacher_classifier(teacher_embeddings)

        # Forward pass on Student model
        student_embeddings = self.modules.student_embedding_model(feats, lens)
        student_embeddings = self.modules.student_mean_var_norm_emb(
                x=student_embeddings, 
                lengths=torch.ones(student_embeddings.shape[0], device=student_embeddings.device)
            )
        student_outputs = self.modules.student_classifier(student_embeddings)
    ...
    def compute_objectives(self, inputs, batch, stage):
        ...
        loss_a = self.hparams.compute_cost(student_predictions, targets)
        loss_inter = compute_loss_inter(self.modules.student_classifier)

        loss = (1.0 - lambda_inter) * loss_a + lambda_inter * loss_inter
        ...
        self.loss_metric.append(
            batch.id, student_predictions, targets, reduction="batch"
        )

        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(
                batch.id, student_predictions, targets
            )
    ...
    def predict_batch(self, batch, topk=1):
        output = self.compute_forward(batch, stage=sb.Stage.TEST)[-2]
...
if __name__ == "__main__":
    ...
    # Collect and load pretrained teacher modules
    sb.utils.distributed.run_on_main(
